...
**0.8.3 is available.** Customize Blockly categories with a hue wheel and matching help colors, refresh newly copied Portal experiences without reconnecting, and import experiences reliably without a page-reload race. The matching [High Poly add-on](https://github.com/TabbedScamper/BF6_Unreal_SDK_High_Poly) reduces CPU memory use with exact texture backing and hidden map-buffer recovery. Install it from the map selector; update both through the shared update button. See [what's new](Plugins/BF6UnrealSDK/Resources/CHANGELOG.md), [block colors](Plugins/BF6UnrealSDK/docs/BLOCK-COLORS.md), and [safe updates and block recovery](Plugins/BF6UnrealSDK/docs/BLOCK-RECOVERY.md). Save/export unsaved Blocks edits before updating from 0.8.1 or earlier.

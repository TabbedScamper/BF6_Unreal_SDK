...
**0.8.2 is available.** Fixes experience blocks opening empty and adds verified block recovery before update restarts. If updating from 0.8.1 or earlier, save/export unsaved Blocks edits first. See [safe updates and block recovery](Plugins/BF6UnrealSDK/docs/BLOCK-RECOVERY.md). Smoother navigation in large block projects, single-script Portal export, adjustable block readability, restored backup transforms, and editable weapon and gadget cards. The matching [High Poly add-on](https://github.com/TabbedScamper/BF6_Unreal_SDK_High_Poly) adds texture streaming, viewport quality presets, attachment previews, and fixes for water, mirrored objects and map-unload crashes. Install or update both from the tool. See [what's new](Plugins/BF6UnrealSDK/Resources/CHANGELOG.md), the [Portal export guide](Plugins/BF6UnrealSDK/docs/PORTAL-EXPORT.md) and the [loadout guide](Plugins/BF6UnrealSDK/docs/LOADOUTS.md).
